...
export abstract class Tables {
    ...
    migrate(): Promise<void> {
        return new Promise<void>(async (resolve, reject) => {
            ...
            // Serialize??
            let data = await this.raw_knex('db_table_versions')
            ...
            for (var [table, latest] of versions) {
                ...
                if (!current_versions.has(table_id)) {
                    ...
                    let err = await this.raw_knex('db_table_versions')
                        .insert({
                            table_id,
                    ...
                    if (err)
                        return reject(`Failed adding table version identifier. ${err}`)
                ...
                while (true) {
                    ...
                    try {
                        ...
                        if (migration instanceof Error)
                            return reject(migration)

                        await migration()
                    }
                    catch (err) {
                        return reject(`Failed upgrading table ${table_id} from ${current_version} to ${new_version}. ${err}`)
                    }
